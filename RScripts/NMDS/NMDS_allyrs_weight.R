...
crops.weight<-subset(crops,Measurement=="weight")

#HAVE to convert missing values to zeros
crops.weight$Value[is.na(crops.weight$Value)]<-0


#Extract 3 years
#crops.weight.61.85.09<-subset(crops.weight, Year %in% c(1961,1985,2009))

#Extract all years
#pivot
pivot.allyrs<-dcast(crops.weight, Year+Country ~ Item ,value.var="Value")
...
#stressplot
jpeg('stressplot_allyrs_weight_k2_rep1.jpg')
...
write.csv(summaryOrdiAllyrs, "ordiAllyrsweightRep1.csv")
...
write.csv(diffEllipse, "diffEllipseAllyrsweightRep1.csv")
...
write.csv(NMDS.mean, "centerEllipseAllyrsweightRep1.csv")
...
ggsave("NMDS_ellipse__allyrs_weight_rep1_3Nov2016.png", width=9, height=7, dpi=300)
...
write.csv(MDS1Dist, "MDS1Dist_allyrs_weight_rep1.csv")
write.csv(MDS2Dist, "MDS2Dist_allyrs_weight_rep1.csv")

NMDS$PyDist<-sqrt((NMDS$DistMDS1^2)+(NMDS$DistMDS2^2))
write.csv(NMDS$PyDist, "NMDS$PyDist_allyrs_weight_rep1.csv")

NMDS<-within(NMDS,Year<-factor(NMDS$group,levels=c("1961","1985","2009")))
write.csv(NMDS, "NMDSellipseDist_allyrs_weight_rep1.csv")
