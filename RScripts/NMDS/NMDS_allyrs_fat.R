...
crops.fat<-subset(crops,Measurement=="Fat")

#HAVE to convert missing values to zeros
crops.fat$Value[is.na(crops.fat$Value)]<-0


#Extract 3 years
#crops.fat.61.85.09<-subset(crops.fat, Year %in% c(1961,1985,2009))

#Extract all years
#pivot
pivot.allyrs<-dcast(crops.fat, Year+Country ~ Item ,value.var="Value")
...
#stressplot
jpeg('stressplot_allyrs_fat_k2_rep1_updatedcode.jpg')
...
write.csv(summaryOrdiAllyrs, "ordiAllyrsfatrep1_updatedcode.csv")
...
write.csv(diffEllipse, "diffEllipseAllyrsfatrep1_updatedcode.csv")
...
write.csv(NMDS.mean, "centerEllipseAllyrsfatrep1_updatedcode.csv")
...
ggsave("NMDS_ellipse__allyrs_fat_rep1_updatedcode_3Nov2016.png", width=9, height=7, dpi=300)
...
write.csv(MDS1Dist, "MDS1Dist_allyrs_fat_rep1_updatedcode.csv")
write.csv(MDS2Dist, "MDS2Dist_allyrs_fat_rep1_updatedcode.csv")

NMDS$PyDist<-sqrt((NMDS$DistMDS1^2)+(NMDS$DistMDS2^2))
write.csv(NMDS$PyDist, "NMDS$PyDist_allyrs_fat_rep1_updatedcode.csv")

NMDS<-within(NMDS,Year<-factor(NMDS$group))
write.csv(NMDS, "NMDSellipseDist_allyrs_fat_rep1_updatedcode.csv")
