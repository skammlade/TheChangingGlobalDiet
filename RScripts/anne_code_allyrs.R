...
library(xlsx)
#write.csv(crops.kcal, "c:/users/sara/crops.kcal.csv")

#HAVE to convert missing values to zeros
crops.kcal$Value[is.na(crops.kcal$Value)]<-0


#Extract all years, just calories


pivot.allyrs<-dcast(crops.kcal, Year+Country ~ Item ,value.var="Value")

data4nmds.allyrs<-pivot.allyrs[,3:54]
...
(summary(ordi.allyrs)[3,3]-summary(ordi.allyrs)[3,1])/summary(ordi.allyrs)[3,1] #-0.6879 or a 68.79% decrease in ellipse area between yr 1 and 3


# USING GGPLOT CODE

nmds.groups.allyrs<-pivot.61.85.09[,1:2]
...
NMDS<-data.frame(MDS1 = crops.mds.allyrs$points[,1], MDS2 = crops.mds.allyrs$points[,2],group=nmds.groups.allyrs$Year,Country=nmds.groups.allyrs$Country)
...
write.csv(abund.per.crop,  "c:users/sara/abund.per.crop.csv")