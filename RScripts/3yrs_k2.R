...
#Extract 3 years
crops.kcal.61.85.09<-subset(crops.kcal, Year %in% c(1961,1985,2009))

pivot.61.85.09<-dcast(crops.kcal.61.85.09, Year+Country ~ Item ,value.var="Value")

data4nmds.3yrs.k2<-pivot.61.85.09[,3:54]

library(vegan)

crops.mds.3yrs.k2<-metaMDS(data4nmds.3yrs.k2,distance="bray",k=2) # can play around with options here to ensure convergence
stressplot(crops.mds.3yrs.k2)
#Square root transformation
#Wisconsin double standardization

head(crops.mds.3yrs.k2$diss)
...
cols.3yrs.k2<-rep(c("firebrick3","midnightblue","black"),c(152,152,152))

#Get the species and site scores
spp.sc<-scores(crops.mds.3yrs.k2,display="species",shrink=shrink)
country.sc<-scores(crops.mds.3yrs.k2,display="sites")
#Work out the ranges
...
text(country.sc,col=cols.3yrs.k2,labels="*",cex=1.5)
...
groups2<-factor(c(rep(1,152),rep(2,152),rep(3,152)),labels=c("1961","1985","2007"))

ordi.3yrs.k2<-ordiellipse(crops.mds.3yrs.k2,groups2,display="sites",kind="sd",conf=0.95,draw="lines",label=FALSE,col=c("black"))

str(ordi.3yrs.k2)
summary(ordi.3yrs.k2)
...
(summary(ordi.3yrs.k2)[3,3]-summary(ordi.3yrs.k2)[3,1])/summary(ordi.3yrs.k2)[3,1] #-0.6879 or a 68.79% decrease in ellipse area between yr 1 and 3


# USING GGPLOT CODE

nmds.groups.3yrs.k2<-pivot.61.85.09[,1:2]
#nmds.groups.3yrs.k2<-factor(c(rep(1,152),rep(2,152),rep(3,152)),labels=c("1961","1985","2007"))

#NMDS<-data.frame(MDS1 = crops.mds.3yrs.k2$points[,1], MDS2 = crops.mds.3yrs.k2$points[,2])
NMDS<-data.frame(MDS1 = crops.mds.3yrs.k2$points[,1], MDS2 = crops.mds.3yrs.k2$points[,2],group=nmds.groups.3yrs.k2$Year,Country=nmds.groups.3yrs.k2$Country)
